...
public class TwoSequenceSelect {
    ...
    public static Integer[] find(Integer[] a, Integer[] b, int k) {
        Integer low = Math.max(0, k - b.length); // Minimum elements taken from a
        Integer high = Math.min(k, a.length); // Maximum elements taken from a
    ...
    // public static <T extends Comparable<T>> int[] findInPlace(T[] source, int start, int mid, int end, int k) {
    ...
    //             // a[ia-1] is too large, reduce ia
    //             high = dynamicMid;
    //         } else if (ib > 0 && ia < source.length && source[ib - 1].compareTo(source[ia]) > 0) {
    //             // b[ib-1] is too large, increase ia
    //             low = dynamicMid + 1;
    //         } else {
    //             // Found valid partition
